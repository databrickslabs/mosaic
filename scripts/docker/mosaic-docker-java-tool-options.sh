...
# [2] run this in root of (mosaic repo), e.g. `sh scripts/docker/mosaic-docker-java-tool-options.sh`
# - for IDE driven or Jupyter notebook testing
# [3] if you want to run tests within the container shell
# - [a] `unset JAVA_TOOL_OPTIONS` is needed to execute JVM tests (if using `mosaic-docker-java-tool-options.sh`)
