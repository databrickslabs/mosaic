...
# [4] get shell with, e.g. `docker exec -it spatial-dev /bin/bash -c "unset JAVA_TOOL_OPTIONS && cd /root/mosaic && /bin/bash"`,
# - can have multiple shells going; call `sh scripts/docker/spatial-dev/exec-shell.sh` also
# [5] `docker stop spatial-dev` whenever done to terminate the container
...
docker run -q --privileged --platform linux/amd64 --name spatial-dev -p 8888:8888 -v $PWD:/root/mosaic \
...
docker exec -it spatial-dev /bin/bash -c "cd /root/mosaic && /bin/bash"