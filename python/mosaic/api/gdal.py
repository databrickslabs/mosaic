...
def enable_gdal(spark: SparkSession, with_checkpoint_dir: str = None) -> None:
    """
    ...
    try:
        if with_checkpoint_dir is not None:
            ...
            spark.conf.set(
                "spark.databricks.labs.mosaic.raster.checkpoint", with_checkpoint_dir
            )
            refresh_context()
            config.mosaic_context.jEnableGDAL(
                spark, with_checkpoint_dir=with_checkpoint_dir
            )
        else:
        ...
        if with_checkpoint_dir:
            print(
                f"checkpoint path '{with_checkpoint_dir}' configured for this session."
            )
...
def update_checkpoint_dir(spark: SparkSession, dir: str):
    """
...
def get_checkpoint_dir() -> str:
    """
    This is run on the driver, assumes enable.py already invoked.
    :return: checkpoint directory or exception.
    """
    return config.mosaic_context.get_checkpoint_dir()


def get_checkpoint_dir_default() -> str:
    """
    This is run on the driver, assumes enable.py already invoked.
    :return: default checkpoint directory.
    """
    return config.mosaic_context.get_checkpoint_dir_default()


def is_gdal_enabled() -> bool:
    """
    This is run on the driver, assumes enable.py already invoked.
    :return: has gdal been enabled already?
    """
    return config.mosaic_context.is_gdal_enabled()


def is_manual_mode() -> bool:
    """
    This is run on the driver, assumes enable.py already invoked.
    :return: configured manual mode (effectively file age-off policy of -1).
    """
    return config.mosaic_context.is_manual_mode()
...
def get_cleanup_age_limit_minutes() -> int:
    """
    This is run on the driver, assumes enable.py already invoked.
    :return: configured local file age limit for cleanUp Thread.
    """
    return config.mosaic_context.get_cleanup_age_limit_minutes
