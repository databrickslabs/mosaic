...
GDAL_INIT_SCRIPT_FILENAME = "mosaic-gdal-init.sh"
...
class GDALInstaller:
    ...
    def copy_objects(self):
        api.setup_gdal(self._temp_dir.name, script_out_name=GDAL_INIT_SCRIPT_FILENAME)

    def run_init_script(self):
        gdal_install_script_target = os.path.join(
            self._temp_dir.name, GDAL_INIT_SCRIPT_FILENAME
        )
