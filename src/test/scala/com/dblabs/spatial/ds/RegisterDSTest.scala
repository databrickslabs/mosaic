...
class RegisterDSTest extends PlanTest with SilentSparkSession {
    ...
    test("Register DS should register all packages on  load()") {
        ...
        spark.sql("show functions like 'bng_*'").count() should be > 0L
        spark.sql("show functions like 'ST_*'").count() should be > 0L
        spark.sql("show functions like 'rst_*'").count() should be > 0L
    }
