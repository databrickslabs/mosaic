...
class WKBToWKT extends FunSuite with SparkTest with Matchers {
  test("Conversion from Hex to WKT") {
    ...
    ).collect().map(_.toSeq.head).map(s => Conversions.wkt2geom(UTF8String.fromString(s.asInstanceOf[String])))
    val right = wkt_rows.map(_.head).map(s => Conversions.wkt2geom(UTF8String.fromString(s)))
    ...
    val left2 = spark.sql(
      "select wkb_to_wkt(wkt_to_wkb(wkt)) as wkt from format_testing"
    ).collect().map(_.toSeq.head).map(s => Conversions.wkt2geom(UTF8String.fromString(s.asInstanceOf[String])))
