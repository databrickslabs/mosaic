...
class ST_TransformTest extends QueryTest with SharedSparkSession with ST_TransformBehaviors {
    ...
    test("Testing stTransform (H3, JTS) NO_CODEGEN") { noCodegen { transformBehaviour(H3IndexSystem, JTS) } }
    test("Testing stTransform (H3, ESRI) NO_CODEGEN") { noCodegen { transformBehaviour(H3IndexSystem, ESRI) } }
    test("Testing stTransform (BNG, JTS) NO_CODEGEN") { noCodegen { transformBehaviour(BNGIndexSystem, JTS) } }
    test("Testing stTransform (BNG, ESRI) NO_CODEGEN") { noCodegen { transformBehaviour(BNGIndexSystem, ESRI) } }
