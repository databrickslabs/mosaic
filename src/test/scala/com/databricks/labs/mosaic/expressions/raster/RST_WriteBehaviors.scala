...
trait RST_WriteBehaviors extends QueryTest {
    ...
    def behaviors(indexSystem: IndexSystem, geometryAPI: GeometryAPI, exprConfigOpt: Option[ExprConfig]): Unit = {
        ...
        val writeDir = "/tmp/mosaic_tmp_write" // <-  "mosaic_tmp" to avoid having it deleted
        ...
        val rasterDf = spark.read
            ...
            //.drop("content")
        rasterDf.limit(1).show()

        // test write path tiles (scala for this)
        // - StringType works:
        //   `.withColumn("tile", rst_maketiles($"path", "no_driver", -1, withCheckpoint = true))`
        // - BinaryType does not work:
        //   `.withColumn("tile", rst_maketiles($"path")`
        val gridTiles1Df = rasterDf
            .withColumn("tile", rst_maketiles($"path"))
            .filter(!rst_isempty($"tile"))
            .select(rst_write($"tile", writeDir).alias("tile"))
        gridTiles1Df.limit(1).show()

        val gridTile1 = RasterTestHelpers.getFirstTile(gridTiles1Df, exprConfigOpt)
        val createInfo1 = gridTile1.raster.getCreateInfo(includeExtras = true)
