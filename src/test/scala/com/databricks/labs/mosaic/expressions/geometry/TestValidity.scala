...
class TestValidity extends AnyFlatSpec with ValidityBehaviors with SparkSuite {
    ...
    "ST_IsValid" should "return true for valid geometries for any index system and any geometry API" in {
        ...
        it should behave like validGeometries(MosaicContext.build(BNGIndexSystem, ESRI), spark)
        it should behave like validGeometries(MosaicContext.build(BNGIndexSystem, JTS), spark)
    }
    ...
    "ST_IsValid" should "return false for invalid geometries for any index system and any geometry API" in {
        ...
        it should behave like invalidGeometries(MosaicContext.build(BNGIndexSystem, ESRI), spark)
        it should behave like invalidGeometries(MosaicContext.build(BNGIndexSystem, JTS), spark)
    }
