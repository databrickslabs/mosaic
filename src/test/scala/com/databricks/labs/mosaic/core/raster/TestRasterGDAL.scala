...
class TestRasterGDAL extends SharedSparkSessionGDAL {
    ...
    test("Raster filter operations are correct.") {
        ...
        val ds = gdalJNI.GetDriverByName("GTiff").Create("/mosaic_tmp/test.tif", 50, 50, 1, gdalconst.gdalconstConstants.GDT_Float32)

        val values = 0 until 50 * 50
        ds.GetRasterBand(1).WriteRaster(0, 0, 50, 50, values.toArray)
        ds.FlushCache()

        var result = MosaicRasterGDAL(ds, "", "", "GTiff", -1).filter(5, "avg").flushCache()
        ...
        result = MosaicRasterGDAL(ds, "", "", "GTiff", -1).filter(5, "mode").flushCache()
        ...
        result = MosaicRasterGDAL(ds, "", "", "GTiff", -1).filter(5, "median").flushCache()
        ...
        result = MosaicRasterGDAL(ds, "", "", "GTiff", -1).filter(5, "min").flushCache()
        ...
        result = MosaicRasterGDAL(ds, "", "", "GTiff", -1).filter(5, "max").flushCache()
