...
class GeometryAPITest extends AnyFunSuite with GeometryAPIBehaviors {
    ...
    test("JTS Geometry API") {
        ...
        jts.fromGeoCoord(new GeoCoord(0.2, 0.1)).equals(MosaicPointJTS(Seq(0.1, 0.2))) shouldEqual true
    ...
    test("ESRI Geometry API") {
        ...
        esri.fromGeoCoord(new GeoCoord(0.2, 0.1)).equals(MosaicPointESRI(Seq(0.1, 0.2))) shouldEqual true
    ...
    test("IllegalAPI Geometry API tests") {
        ...
        an[Error] should be thrownBy illegalAPI.fromGeoCoord(new GeoCoord(0.2, 0.1))
