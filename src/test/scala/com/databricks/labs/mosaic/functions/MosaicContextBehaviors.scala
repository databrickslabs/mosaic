...
trait MosaicContextBehaviors extends MosaicSpatialQueryTest {
    ...
    def sqlRegistration(mosaicContext: MosaicContext): Unit = {
        ...
        noException should be thrownBy getFunc("st_bufferdisc").apply(Seq(multiPolygon.expr, xLit, yLit))
        ...
        noException should be thrownBy getFunc("grid_cellkdisc").apply(Seq(multiPolygon.expr, lit(5).expr))
        noException should be thrownBy getFunc("grid_cellkdiscexplode").apply(Seq(multiPolygon.expr, lit(5).expr))
        ...
        noException should be thrownBy getFunc("grid_geometrykdisc").apply(Seq(multiPolygon.expr, lit(5).expr, lit(5).expr))
        noException should be thrownBy getFunc("grid_geometrykdiscexplode").apply(Seq(multiPolygon.expr, lit(5).expr, lit(5).expr))
