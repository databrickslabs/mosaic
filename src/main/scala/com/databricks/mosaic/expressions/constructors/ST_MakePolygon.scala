...
@ExpressionDescription(
  usage = "_FUNC_(expr1) - Creates a new Polygon geometry from a closed LineString.",
  examples = """
    Examples:
      > SELECT _FUNC_(A);
  """,
...
case class ST_MakePolygon(boundaryRing: Expression) extends UnaryExpression with NullIntolerant with CodegenFallback {
    ...
    def isClosed(lineString: InternalGeometry): Boolean = lineString.boundaries.head.head == lineString.boundaries.head.last
