...
case class ST_ConvexHull(inputGeom: Expression, geometryAPIName: String)
  ...
  override def nullSafeEval(inputRow: Any): Any = {
    ...
    if (geometry.getGeometryType != "MultiPoint") {
      ...
      s"Geometry provided to ST_ConvexHull must be MultiPoint (was ${geometry.getGeometryType}).")
    }
