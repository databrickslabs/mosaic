...
class MosaicContext(indexSystem: IndexSystem, geometryAPI: GeometryAPI) extends Serializable with Logging {
    ...
    object functions extends Serializable {
        ...
        def rst_fromcontent(raster: Column, driver:Column): Column = rst_fromcontent(raster, driver, lit(-1).Column)
        def rst_fromcontent(raster: Column, driver:String): Column = rst_fromcontent(raster, lit(driver), lit(-1).Column)
        def rst_fromcontent(raster: Column, driver:Column, sizeInMB:Int): Column = rst_fromcontent(raster, driver, lit(sizeInMB))
        def rst_fromcontent(raster: Column, driver:String, sizeInMB: Int): Column = rst_fromcontent(raster, lit(driver), lit(sizeInMB))
