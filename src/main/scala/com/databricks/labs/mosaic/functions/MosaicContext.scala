...
class MosaicContext(indexSystem: IndexSystem, geometryAPI: GeometryAPI) extends Serializable with Logging {
    ...
    def register(
        ...
        registry.registerFunction(
          ...
          (exprs: Seq[Expression]) =>
              exprs match {
                  case e if e.length == 1 => ST_MakePolygon(e.head, null)
        ...
        registry.registerFunction(
          ...
          (exprs: Seq[Expression]) =>
              exprs match {
                  case e if e.length == 2 => MosaicExplode(e(0), e(1), null, indexSystem, geometryAPI.name)
        ...
        registry.registerFunction(
          ...
          (exprs: Seq[Expression]) => MosaicFill(exprs(0), exprs(1),null, indexSystem, geometryAPI.name)
        )
        registry.registerFunction(
          ...
          (exprs: Seq[Expression]) =>
              exprs match {
                  case e if e.length == 2 => MosaicFill(e(0), e(1),null, indexSystem, geometryAPI.name)
    ...
    object functions extends Serializable {
        ...
        /** IndexSystem and GeometryAPI Agnostic methods */
        def as_hex(inGeom: Column): Column = ColumnAdapter(AsHex(null))
        def as_json(inGeom: Column): Column = ColumnAdapter(AsJSON(null))

        /** GeometryAPI Specific */

        /** Spatial functions */
        def flatten_polygons(geom: Column): Column = ColumnAdapter(FlattenPolygons(null, geometryAPI.name))
        def st_area(geom: Column): Column = ColumnAdapter(ST_Area(null, expressionConfig))
        ...
        def st_buffer(geom: Column, radius: Column, buffer_style_parameters: Column): Column =
            ColumnAdapter(ST_Buffer(null, null, null, expressionConfig))
        def st_buffer(geom: Column, radius: Double, buffer_style_parameters: Column): Column =
            ColumnAdapter(
              ST_Buffer(null, null, null, expressionConfig)
            )
        def st_bufferloop(geom: Column, r1: Column, r2: Column): Column =
            ColumnAdapter(ST_BufferLoop(null, null, null, expressionConfig))
        ...
        def st_buffer_cap_style(geom: Column, radius: Column, capStyle: Column): Column =
            ColumnAdapter(ST_BufferCapStyle(null, null, null, expressionConfig))
        def st_buffer_cap_style(geom: Column, radius: Double, capStyle: String): Column =
            ColumnAdapter(ST_BufferCapStyle(null, null, null, expressionConfig))
        def st_centroid(geom: Column): Column = ColumnAdapter(ST_Centroid(null, expressionConfig))
        def st_convexhull(geom: Column): Column = ColumnAdapter(ST_ConvexHull(null, expressionConfig))
        def st_concavehull(geom: Column, concavity: Column, allowHoles: Column): Column =
            ColumnAdapter(ST_ConcaveHull(null, null, null, expressionConfig))
        ...
        def st_concavehull(geom: Column, concavity: Double): Column =
            ColumnAdapter(ST_ConcaveHull(null, null, null, expressionConfig))
        def st_difference(geom1: Column, geom2: Column): Column = ColumnAdapter(ST_Difference(null, null, expressionConfig))
        def st_distance(geom1: Column, geom2: Column): Column = ColumnAdapter(ST_Distance(null, null, expressionConfig))
        def st_dimension(geom: Column): Column = ColumnAdapter(ST_Dimension(null, expressionConfig))
        def st_dump(geom: Column): Column = ColumnAdapter(FlattenPolygons(null, geometryAPI.name))
        def st_envelope(geom: Column): Column = ColumnAdapter(ST_Envelope(null, expressionConfig))
        def st_geometrytype(geom: Column): Column = ColumnAdapter(ST_GeometryType(null, expressionConfig))
        def st_hasvalidcoordinates(geom: Column, crsCode: Column, which: Column): Column =
            ColumnAdapter(ST_HasValidCoordinates(null, null, null, expressionConfig))
        def st_interpolateelevation(pointsArray: Column, linesArray: Column, mergetol: Column, snaptol: Column, splitPointFinder: Column, origin: Column, xWidth: Column, yWidth: Column, xSize: Column, ySize: Column): Column =
            ColumnAdapter(geometry.ST_InterpolateElevation(null, null, null, null, null, null, null, null, null, null, expressionConfig))
        def st_intersection(left: Column, right: Column): Column = ColumnAdapter(ST_Intersection(null, null, expressionConfig))
        def st_isvalid(geom: Column): Column = ColumnAdapter(ST_IsValid(null, expressionConfig))
        def st_length(geom: Column): Column = ColumnAdapter(ST_Length(null, expressionConfig))
        def st_numpoints(geom: Column): Column = ColumnAdapter(ST_NumPoints(null, expressionConfig))
        def st_perimeter(geom: Column): Column = ColumnAdapter(ST_Length(null, expressionConfig))

        def st_haversine(lat1: Column, lon1: Column, lat2: Column, lon2: Column): Column =
            ColumnAdapter(ST_Haversine(null, null, null, null))

        def st_rotate(geom1: Column, td: Column): Column = ColumnAdapter(ST_Rotate(null, null, expressionConfig))
        def st_scale(geom1: Column, xd: Column, yd: Column): Column =
            ColumnAdapter(ST_Scale(null, null, null, expressionConfig))
        def st_setsrid(geom: Column, srid: Column): Column = ColumnAdapter(ST_SetSRID(null, null, expressionConfig))
        def st_simplify(geom: Column, tolerance: Column): Column =
            ColumnAdapter(ST_Simplify(null, null, expressionConfig))
        def st_simplify(geom: Column, tolerance: Double): Column =
            ColumnAdapter(ST_Simplify(null, null, expressionConfig))
        def st_srid(geom: Column): Column = ColumnAdapter(ST_SRID(null, expressionConfig))
        def st_transform(geom: Column, srid: Column): Column = ColumnAdapter(ST_Transform(null, null, expressionConfig))
        def st_translate(geom1: Column, xd: Column, yd: Column): Column =
            ColumnAdapter(ST_Translate(null, null, null, expressionConfig))
        def st_triangulate(pointsArray: Column, linesArray: Column, mergeTol: Column, snapTol: Column, splitPointFinder: Column): Column =
            ColumnAdapter(ST_Triangulate(null, null, null, null, null, expressionConfig))
        def st_x(geom: Column): Column = ColumnAdapter(ST_X(null, expressionConfig))
        def st_y(geom: Column): Column = ColumnAdapter(ST_Y(null, expressionConfig))
        def st_z(geom: Column): Column = ColumnAdapter(ST_Z(null, expressionConfig))
        def st_xmax(geom: Column): Column = ColumnAdapter(ST_MinMaxXYZ(null, expressionConfig, "X", "MAX"))
        def st_xmin(geom: Column): Column = ColumnAdapter(ST_MinMaxXYZ(null, expressionConfig, "X", "MIN"))
        def st_ymax(geom: Column): Column = ColumnAdapter(ST_MinMaxXYZ(null, expressionConfig, "Y", "MAX"))
        def st_ymin(geom: Column): Column = ColumnAdapter(ST_MinMaxXYZ(null, expressionConfig, "Y", "MIN"))
        def st_zmax(geom: Column): Column = ColumnAdapter(ST_MinMaxXYZ(null, expressionConfig, "Z", "MAX"))
        def st_zmin(geom: Column): Column = ColumnAdapter(ST_MinMaxXYZ(null, expressionConfig, "Z", "MIN"))
        def st_union(leftGeom: Column, rightGeom: Column): Column = ColumnAdapter(ST_Union(null, null, expressionConfig))
        def st_unaryunion(geom: Column): Column = ColumnAdapter(ST_UnaryUnion(null, expressionConfig))
        def st_updatesrid(geom: Column, srcSRID: Column, destSRID: Column): Column =
            ColumnAdapter(ST_UpdateSRID(null, null, null, expressionConfig))
        def st_updatesrid(geom: Column, srcSRID: Int, destSRID: Int): Column =
            ColumnAdapter(ST_UpdateSRID(null, null, null, expressionConfig))

        /** Undocumented helper */
        def convert_to(inGeom: Column, outDataType: String): Column =
            ColumnAdapter(ConvertTo(null, outDataType, geometryAPI.name, Some("convert_to")))

        /** Geometry constructors */
        def st_point(xVal: Column, yVal: Column): Column = ColumnAdapter(ST_Point(null, null))
        def st_geomfromwkt(inGeom: Column): Column =
            ColumnAdapter(ConvertTo(null, "coords", geometryAPI.name, Some("st_geomfromwkt")))
        def st_geomfromwkb(inGeom: Column): Column =
            ColumnAdapter(ConvertTo(null, "coords", geometryAPI.name, Some("st_geomfromwkb")))
        def st_geomfromgeojson(inGeom: Column): Column =
            ColumnAdapter(ConvertTo(AsJSON(null), "coords", geometryAPI.name, Some("st_geomfromgeojson")))
        def st_makeline(points: Column): Column = ColumnAdapter(ST_MakeLine(null, geometryAPI.name))
        def st_makepolygon(boundaryRing: Column): Column = ColumnAdapter(ST_MakePolygon(null, null))
        def st_makepolygon(boundaryRing: Column, holeRingArray: Column): Column =
            ColumnAdapter(ST_MakePolygon(null, null))

        /** Geometry accessors */
        def st_asbinary(geom: Column): Column = ColumnAdapter(ConvertTo(null, "wkb", geometryAPI.name, Some("st_asbinary")))
        def st_asgeojson(geom: Column): Column = ColumnAdapter(ConvertTo(null, "geojson", geometryAPI.name, Some("st_asgeojson")))
        def st_astext(geom: Column): Column = ColumnAdapter(ConvertTo(null, "wkt", geometryAPI.name, Some("st_astext")))
        def st_aswkb(geom: Column): Column = ColumnAdapter(ConvertTo(null, "wkb", geometryAPI.name, Some("st_aswkb")))
        def st_aswkt(geom: Column): Column = ColumnAdapter(ConvertTo(null, "wkt", geometryAPI.name, Some("st_aswkt")))

        /** Spatial predicates */
        def st_contains(geom1: Column, geom2: Column): Column = ColumnAdapter(ST_Contains(null, null, expressionConfig))
        def st_intersects(left: Column, right: Column): Column = ColumnAdapter(ST_Intersects(null, null, expressionConfig))
        def st_within(geom1: Column, geom2: Column): Column = ColumnAdapter(ST_Within(null, null, expressionConfig))

        /** RasterAPI dependent functions */
        def rst_asformat(raster: Column, driver: Column): Column =
            ColumnAdapter(RST_AsFormat(null, null, expressionConfig))
        def rst_asformat(raster: Column, driver: String): Column =
            ColumnAdapter(RST_AsFormat(null, null, expressionConfig))
        def rst_bandmetadata(raster: Column, band: Column): Column =
            ColumnAdapter(RST_BandMetaData(null, null, expressionConfig))
        def rst_bandmetadata(raster: Column, band: Int): Column =
            ColumnAdapter(RST_BandMetaData(null, null, expressionConfig))
        def rst_boundingbox(raster: Column): Column = ColumnAdapter(RST_BoundingBox(null, expressionConfig))
        def rst_clip(raster: Column, geometry: Column): Column = ColumnAdapter(RST_Clip(null, null,null, expressionConfig))
        def rst_clip(raster: Column, geometry: Column, cutline: Boolean): Column = ColumnAdapter(RST_Clip(null, null, null, expressionConfig))
        def rst_clip(raster: Column, geometry: Column, cutline: Column): Column = ColumnAdapter(RST_Clip(null, null, null, expressionConfig))
        def rst_convolve(raster: Column, kernel: Column): Column = ColumnAdapter(RST_Convolve(null, null, expressionConfig))
        ...
        def rst_dtmfromgeoms(pointsArray: Column, linesArray: Column, mergeTol: Column, snapTol: Column, splitPointFinder: Column, origin: Column, xWidth: Column, yWidth: Column, xSize: Column, ySize: Column, noData: Column): Column =
            ColumnAdapter(RST_DTMFromGeoms(null, null, null, null, null, null, null, null, null, null, null, expressionConfig))
        def rst_pixelcount(raster: Column): Column = ColumnAdapter(RST_PixelCount(null, null, null, expressionConfig))
        def rst_pixelcount(raster: Column, countNoData: Column): Column = ColumnAdapter(RST_PixelCount(null, null, null, expressionConfig))
        def rst_pixelcount(raster: Column, countNoData: Column, countAll: Column): Column = ColumnAdapter(RST_PixelCount(null, null, null, expressionConfig))
        def rst_combineavg(rasterArray: Column): Column = ColumnAdapter(RST_CombineAvg(null, expressionConfig))
        def rst_derivedband(raster: Column, pythonFunc: Column, funcName: Column): Column =
            ColumnAdapter(RST_DerivedBand(null, null, null, expressionConfig))
        def rst_filter(raster: Column, kernelSize: Column, operation: Column): Column =
            ColumnAdapter(RST_Filter(null, null, null, expressionConfig))
        def rst_filter(raster: Column, kernelSize: Int, operation: String): Column =
            ColumnAdapter(RST_Filter(null, null, null, expressionConfig))
        def rst_format(raster: Column): Column =
            ColumnAdapter(RST_Format(null, expressionConfig))
        def rst_georeference(raster: Column): Column = ColumnAdapter(RST_GeoReference(null, expressionConfig))
        def rst_getnodata(raster: Column): Column = ColumnAdapter(RST_GetNoData(null, expressionConfig))
        def rst_getsubdataset(raster: Column, subdatasetName: Column): Column =
            ColumnAdapter(RST_GetSubdataset(null, null, expressionConfig))
        def rst_getsubdataset(raster: Column, subdatasetName: String): Column =
            ColumnAdapter(RST_GetSubdataset(null, null, expressionConfig))
        def rst_height(raster: Column): Column = ColumnAdapter(RST_Height(null, expressionConfig))
        def rst_initnodata(raster: Column): Column = ColumnAdapter(RST_InitNoData(null, expressionConfig))
        def rst_isempty(raster: Column): Column = ColumnAdapter(RST_IsEmpty(null, expressionConfig))
        def rst_maketiles(input: Column, driver: Column, size: Column, withCheckpoint: Column): Column =
            ColumnAdapter(RST_MakeTiles(null, null, null, null, expressionConfig))
        ...
        def rst_maketiles(input: Column): Column =
            ColumnAdapter(RST_MakeTiles(null, null, null, null, expressionConfig))
        def rst_max(raster: Column): Column = ColumnAdapter(RST_Max(null, expressionConfig))
        def rst_min(raster: Column): Column = ColumnAdapter(RST_Min(null, expressionConfig))
        def rst_median(raster: Column): Column = ColumnAdapter(RST_Median(null, expressionConfig))
        def rst_avg(raster: Column): Column = ColumnAdapter(RST_Avg(null, expressionConfig))
        def rst_memsize(raster: Column): Column = ColumnAdapter(RST_MemSize(null, expressionConfig))
        def rst_frombands(bandsArray: Column): Column = ColumnAdapter(RST_FromBands(null, expressionConfig))
        def rst_merge(rasterArray: Column): Column = ColumnAdapter(RST_Merge(null, expressionConfig))
        def rst_metadata(raster: Column): Column = ColumnAdapter(RST_MetaData(null, expressionConfig))
        def rst_mapalgebra(rasterArray: Column, jsonSpec: Column): Column =
            ColumnAdapter(RST_MapAlgebra(null, null, expressionConfig))
        def rst_ndvi(raster: Column, band1: Column, band2: Column): Column =
            ColumnAdapter(RST_NDVI(null, null, null, expressionConfig))
        def rst_ndvi(raster: Column, band1: Int, band2: Int): Column =
            ColumnAdapter(RST_NDVI(null, null, null, expressionConfig))
        def rst_numbands(raster: Column): Column = ColumnAdapter(RST_NumBands(null, expressionConfig))
        def rst_pixelheight(raster: Column): Column = ColumnAdapter(RST_PixelHeight(null, expressionConfig))
        def rst_pixelwidth(raster: Column): Column = ColumnAdapter(RST_PixelWidth(null, expressionConfig))
        def rst_rastertogridavg(raster: Column, resolution: Column): Column =
            ColumnAdapter(RST_RasterToGridAvg(null, null, expressionConfig))
        def rst_rastertogridcount(raster: Column, resolution: Column): Column =
            ColumnAdapter(RST_RasterToGridCount(null, null, expressionConfig))
        def rst_rastertogridmax(raster: Column, resolution: Column): Column =
            ColumnAdapter(RST_RasterToGridMax(null, null, expressionConfig))
        def rst_rastertogridmedian(raster: Column, resolution: Column): Column =
            ColumnAdapter(RST_RasterToGridMedian(null, null, expressionConfig))
        def rst_rastertogridmin(raster: Column, resolution: Column): Column =
            ColumnAdapter(RST_RasterToGridMin(null, null, expressionConfig))
        def rst_rastertoworldcoord(raster: Column, x: Column, y: Column): Column =
            ColumnAdapter(RST_RasterToWorldCoord(null, null, null, expressionConfig))
        ...
        def rst_rastertoworldcoordx(raster: Column, x: Column, y: Column): Column =
            ColumnAdapter(RST_RasterToWorldCoordX(null, null, null, expressionConfig))
        ...
        def rst_rastertoworldcoordy(raster: Column, x: Column, y: Column): Column =
            ColumnAdapter(RST_RasterToWorldCoordY(null, null, null, expressionConfig))
        ...
        def rst_retile(raster: Column, tileWidth: Column, tileHeight: Column): Column =
            ColumnAdapter(RST_ReTile(null, null, null, expressionConfig))
        def rst_retile(raster: Column, tileWidth: Int, tileHeight: Int): Column =
            ColumnAdapter(RST_ReTile(null, null, null, expressionConfig))
        def rst_separatebands(raster: Column): Column = ColumnAdapter(RST_SeparateBands(null, expressionConfig))
        def rst_rotation(raster: Column): Column = ColumnAdapter(RST_Rotation(null, expressionConfig))
        def rst_scalex(raster: Column): Column = ColumnAdapter(RST_ScaleX(null, expressionConfig))
        def rst_scaley(raster: Column): Column = ColumnAdapter(RST_ScaleY(null, expressionConfig))
        def rst_setnodata(raster: Column, nodata: Column): Column = ColumnAdapter(RST_SetNoData(null, null, expressionConfig))
        def rst_setnodata(raster: Column, nodata: Double): Column =
            ColumnAdapter(RST_SetNoData(null, null, expressionConfig))
        def rst_skewx(raster: Column): Column = ColumnAdapter(RST_SkewX(null, expressionConfig))
        def rst_skewy(raster: Column): Column = ColumnAdapter(RST_SkewY(null, expressionConfig))
        def rst_srid(raster: Column): Column = ColumnAdapter(RST_SRID(null, expressionConfig))
        def rst_setsrid(raster: Column, srid: Column): Column = ColumnAdapter(RST_SetSRID(null, null, expressionConfig))
        def rst_subdatasets(raster: Column): Column = ColumnAdapter(RST_Subdatasets(null, expressionConfig))
        def rst_summary(raster: Column): Column = ColumnAdapter(RST_Summary(null, expressionConfig))
        def rst_tessellate(raster: Column, resolution: Column): Column =
            ColumnAdapter(RST_Tessellate(null, null, expressionConfig))
        def rst_transform(raster: Column, srid: Column): Column = ColumnAdapter(RST_Transform(null, null, expressionConfig))
        def rst_tessellate(raster: Column, resolution: Int): Column =
            ColumnAdapter(RST_Tessellate(null, null, expressionConfig))
        def rst_type(raster: Column): Column = ColumnAdapter(RST_Type(null, expressionConfig))
        def rst_fromcontent(raster: Column, driver: Column): Column =
            ColumnAdapter(RST_FromContent(null, null, null, expressionConfig))
        ...
        def rst_fromcontent(raster: Column, driver: String, sizeInMB: Int): Column =
            ColumnAdapter(RST_FromContent(null, null, null, expressionConfig))
        def rst_fromfile(raster: Column): Column = ColumnAdapter(RST_FromFile(null, null, expressionConfig))
        def rst_fromfile(raster: Column, sizeInMB: Column): Column =
            ColumnAdapter(RST_FromFile(null, null, expressionConfig))
        ...
        def rst_tooverlappingtiles(raster: Column, width: Int, height: Int, overlap: Int): Column =
            ColumnAdapter(RST_ToOverlappingTiles(null, null, null, null, expressionConfig))
        def rst_tooverlappingtiles(raster: Column, width: Column, height: Column, overlap: Column): Column =
            ColumnAdapter(RST_ToOverlappingTiles(null, null, null, null, expressionConfig))
        def rst_tryopen(raster: Column): Column = ColumnAdapter(RST_TryOpen(null, expressionConfig))
        def rst_subdivide(raster: Column, sizeInMB: Column): Column =
            ColumnAdapter(RST_Subdivide(null, null, expressionConfig))
        def rst_subdivide(raster: Column, sizeInMB: Int): Column =
            ColumnAdapter(RST_Subdivide(null, null, expressionConfig))
        def rst_updatetype(raster: Column, dataType: Column): Column =
            ColumnAdapter(RST_UpdateType(null, null, expressionConfig))
        def rst_updatetype(raster: Column, dataType: String): Column =
            ColumnAdapter(RST_UpdateType(null, null, expressionConfig))
        def rst_upperleftx(raster: Column): Column = ColumnAdapter(RST_UpperLeftX(null, expressionConfig))
        def rst_upperlefty(raster: Column): Column = ColumnAdapter(RST_UpperLeftY(null, expressionConfig))
        def rst_width(raster: Column): Column = ColumnAdapter(RST_Width(null, expressionConfig))
        def rst_worldtorastercoord(raster: Column, x: Column, y: Column): Column =
            ColumnAdapter(RST_WorldToRasterCoord(null, null, null, expressionConfig))
        ...
        def rst_worldtorastercoordx(raster: Column, x: Column, y: Column): Column =
            ColumnAdapter(RST_WorldToRasterCoordX(null, null, null, expressionConfig))
        ...
        def rst_worldtorastercoordy(raster: Column, x: Column, y: Column): Column =
            ColumnAdapter(RST_WorldToRasterCoordY(null, null, null, expressionConfig))
        def rst_worldtorastercoordy(raster: Column, x: Double, y: Double): Column =
            ColumnAdapter(RST_WorldToRasterCoordY(null, null, null, expressionConfig))
        def rst_write(raster: Column, path: Column): Column =
            ColumnAdapter(RST_Write(null, null, expressionConfig))
        def rst_write(raster: Column, path: String): Column =
            ColumnAdapter(RST_Write(null, null, expressionConfig))
        /** Aggregators */

        def st_asgeojsontile_agg(geom: Column, attributes: Column): Column =
            ColumnAdapter(ST_AsGeojsonTileAgg(null, null, expressionConfig, 0, 0).toAggregateExpression(isDistinct = false))
        def st_asmvttile_agg(geom: Column, attributes: Column, zxyID: Column): Column =
            ColumnAdapter(
              ST_AsMVTTileAgg(null, null, null, expressionConfig, 0, 0).toAggregateExpression(isDistinct = false)
            )
        def st_intersects_agg(leftIndex: Column, rightIndex: Column): Column =
            ColumnAdapter(
              ST_IntersectsAgg(null, null, geometryAPI.name).toAggregateExpression(isDistinct = false)
            )
        def st_intersection_agg(leftIndex: Column, rightIndex: Column): Column =
            ColumnAdapter(
              ST_IntersectionAgg(null, null, geometryAPI.name, indexSystem, 0, 0)
                  .toAggregateExpression(isDistinct = false)
            )
        def st_union_agg(geom: Column): Column =
            ColumnAdapter(ST_UnionAgg(null, geometryAPI.name).toAggregateExpression(isDistinct = false))
        def rst_merge_agg(raster: Column): Column =
            ColumnAdapter(RST_MergeAgg(null, expressionConfig).toAggregateExpression(isDistinct = false))
        def rst_combineavg_agg(raster: Column): Column =
            ColumnAdapter(RST_CombineAvgAgg(null, expressionConfig).toAggregateExpression(isDistinct = false))
        def rst_derivedband_agg(raster: Column, pythonFunc: Column, funcName: Column): Column =
            ColumnAdapter(
              RST_DerivedBandAgg(null, null, null, expressionConfig).toAggregateExpression(isDistinct = false)
            )
        ...
        def grid_cell_intersection(chip1: Column, chip2: Column): Column =
            ColumnAdapter(CellIntersection(null, null, indexSystem, geometryAPI.name))
        def grid_cell_intersection_agg(chip: Column): Column =
            ColumnAdapter(CellIntersectionAgg(null, geometryAPI.name, indexSystem).toAggregateExpression(isDistinct = false))
        def grid_cell_union(chip1: Column, chip2: Column): Column =
            ColumnAdapter(CellUnion(null, null, indexSystem, geometryAPI.name))
        def grid_cell_union_agg(chip: Column): Column =
            ColumnAdapter(CellUnionAgg(null, geometryAPI.name, indexSystem).toAggregateExpression(isDistinct = false))
        def grid_distance(cell1: Column, cell2: Column): Column =
            ColumnAdapter(GridDistance(null, null, indexSystem, geometryAPI.name))
        ...
        def grid_tessellateexplode(geom: Column, resolution: Column, keepCoreGeometries: Column): Column =
            ColumnAdapter(
              MosaicExplode(null, null, null, indexSystem, geometryAPI.name)
            )
        ...
        def grid_tessellate(geom: Column, resolution: Column, keepCoreGeometries: Column): Column =
            ColumnAdapter(
              MosaicFill(null, null, null, indexSystem, geometryAPI.name)
            )
        def grid_pointascellid(point: Column, resolution: Column): Column =
            ColumnAdapter(PointIndexGeom(null, null, indexSystem, geometryAPI.name))
        ...
        def grid_longlatascellid(lon: Column, lat: Column, resolution: Column): Column = {
            if (shouldUseDatabricksH3()) {
                ...
                ColumnAdapter(PointIndexLonLat(null, null, null, indexSystem))
            }
        }
        ...
        def grid_polyfill(geom: Column, resolution: Column): Column = {
            if (shouldUseDatabricksH3()) {
                ...
                ColumnAdapter(Polyfill(null, null, indexSystem, getGeometryAPI.name))
            }
        }
        ...
        def grid_boundaryaswkb(indexID: Column): Column = {
            if (shouldUseDatabricksH3()) {
                ...
                ColumnAdapter(IndexGeometry(null, null, indexSystem, getGeometryAPI.name))
            }
        }
        def grid_boundary(indexID: Column, format: Column): Column =
            ColumnAdapter(IndexGeometry(null, null, indexSystem, geometryAPI.name))
        def grid_boundary(indexID: Column, format: String): Column =
            ColumnAdapter(IndexGeometry(null, null, indexSystem, geometryAPI.name))
        def grid_cellarea(cellId: Column): Column = ColumnAdapter(CellArea(null, indexSystem, geometryAPI.name))
        def grid_cellkring(cellId: Column, k: Column): Column = ColumnAdapter(CellKRing(null, null, indexSystem, geometryAPI.name))
        def grid_cellkring(cellId: Column, k: Int): Column =
            ColumnAdapter(CellKRing(null, null, indexSystem, geometryAPI.name))
        def grid_cellkringexplode(cellId: Column, k: Int): Column =
            ColumnAdapter(CellKRingExplode(null, null, indexSystem, geometryAPI.name))
        def grid_cellkringexplode(cellId: Column, k: Column): Column =
            ColumnAdapter(CellKRingExplode(null, null, indexSystem, geometryAPI.name))
        def grid_cellkloop(cellId: Column, k: Column): Column = ColumnAdapter(CellKLoop(null, null, indexSystem, geometryAPI.name))
        def grid_cellkloop(cellId: Column, k: Int): Column =
            ColumnAdapter(CellKLoop(null, null, indexSystem, geometryAPI.name))
        def grid_cellkloopexplode(cellId: Column, k: Int): Column =
            ColumnAdapter(CellKLoopExplode(null, null, indexSystem, geometryAPI.name))
        ...
        def grid_geometrykring(geom: Column, resolution: Column, k: Column): Column =
            ColumnAdapter(GeometryKRing(null, null, null, indexSystem, geometryAPI.name))
        ...
        def grid_geometrykringexplode(geom: Column, resolution: Column, k: Column): Column =
            ColumnAdapter(GeometryKRingExplode(null, null, null, indexSystem, geometryAPI.name))
        ...
        def grid_geometrykloop(geom: Column, resolution: Column, k: Column): Column =
            ColumnAdapter(GeometryKLoop(null, null, null, indexSystem, geometryAPI.name))
        ...
        def grid_geometrykloopexplode(geom: Column, resolution: Column, k: Column): Column =
            ColumnAdapter(GeometryKLoopExplode(null, null, null, indexSystem, geometryAPI.name))
        ...
        // Not specific to Mosaic
        def try_sql(inCol: Column): Column = ColumnAdapter(TrySql(null))
        ...
        def st_centroid2D(geom: Column): Column = {
            struct(
              ColumnAdapter(ST_X(ST_Centroid(null, expressionConfig), expressionConfig)),
              ColumnAdapter(ST_Y(ST_Centroid(null, expressionConfig), expressionConfig))
            )
        }
