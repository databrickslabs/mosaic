...
abstract class GeometryPredicateExpression[T <: Expression: ClassTag](
    ...
    /** Output Data Type */
    override def dataType: DataType = leftGeomExpr.dataType

    def geomTransform(leftGeom: MosaicGeometry, rightGeom: MosaicGeometry): Any
    ...
    override def nullSafeEval(input1: Any, input2: Any): Any = {
        ...
        val resultGeom = geomTransform(leftGeom, rightGeom).asInstanceOf[MosaicGeometry]
        geometryAPI.serialize(resultGeom, leftGeomExpr.dataType)
    }
    ...
    override def doGenCode(ctx: CodegenContext, ev: ExprCode): ExprCode = {
        nullSafeCodeGen(
          ...
          (leftEval, rightEval) => {
              ...
              val (outCode, outGeomRef) = ConvertToCodeGen.writeGeometryCode(ctx, resultGeomRef, leftGeomExpr.dataType, geometryAPI)
