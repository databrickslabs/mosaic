...
case class ExpressionConfig(
    ...
    def getGDALConfig: Map[String, String] = {
        configs.filter(p => {
            p._1.startsWith("spark.dblabs.gbx.gdal.") ||
    ...
    def getSharedObjects: Map[String, String] = {
        configs.filter(p => {
            p._1.startsWith("spark.dblabs.gbx.sharedobjects.") ||
            p._1.startsWith("spark.sharedobjects.")
        })
    }

    def getRasterCheckpointDir: String = {
        configs.getOrElse("spark.dblabs.gbx.raster.checkpoint.dir", "/tmp/raster-checkpoint")
    }

    def useCheckpoint: Boolean = {
        configs.getOrElse("spark.dblabs.gbx.raster.use.checkpoint", "false").toBoolean
    }
