...
case class CustomIndexSystem(conf: GridConf) extends Serializable {
    ...
    def kRing(index: Long, k: Int): Seq[Long] = {
    ...
    /**
      ...
      */
    def kLoop(index: Long, k: Int): Seq[Long] = {
    ...
    /**
      ...
      *
      * @param geometry
      *   An instance of [[Geometry]] for which we are computing the
    ...
    def indexToBoundary(index: Long): Seq[Coordinate] = {
        val geometry = indexToGeometry(index)
        if (geometry.isEmpty) {
    ...
    def indexToCenter(index: Long): Coordinate = {
        val geometry = indexToGeometry(index)
        if (geometry.isEmpty) {
            throw new IllegalStateException(s"Index $index does not correspond to a valid geometry.")
        }
